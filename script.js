...
if(timerElement){
    ...
    setInterval(()=>{
        ...
        if(diff<=0){ timerElement.innerHTML="🎉 Today is your day 🎉"; return; }
...
function revealLove(){
    ...
    if(loveDiv) loveDiv.classList.add("show");
}
...
if(canvas){
    ...
    class Firework{
        constructor(x,y,targetY){ this.x=x; this.y=y; this.targetY=targetY; this.color=`hsl(${random(0,360)},100%,50%)`; this.exploded=false; }
        update(){ if(this.y>this.targetY){ this.y -= 8; } else if(!this.exploded){ this.explode(); this.exploded=true; } }
    ...
    class Particle{
        constructor(x,y,color){ this.x=x; this.y=y; this.color=color; this.vx=random(-5,5); this.vy=random(-5,5); this.alpha=1; }
